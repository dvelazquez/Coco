...
int i, M, fd, Timer, Value1, Value2;		/* M is #of Measure */
...
unsigned char sResult[0], sCmd[0], sOP[0];
float iSP;

  FILE *outfile;
  BITMAP *Captura, *Clean, *Screen2, *Disconnected, *CleanBox;
  PALETTE pal;
int Automatic(int argc, unsigned char **argv)
{
  ...
  while (!key[KEY_ESC])	/* Only pressing ESC can Exit */
      ...
      if (ustrcmp(Mode,"P")==0)
	{
	  ...
	  if((SP-PV) =='0') 	// No Error
	    {
	      OP=(368/2);	// Internal OP for plotting
	      sOP[0]= 150;	// OP when E=0 is half scale (Micro's OP)
	      sCmd[0]= 'M';
	      if(!writeport(fd, sCmd))
	         return 1;
	     usleep(500);
	   ...
	   {
	     OP= ((((PV/368)-(SP/368))*100)*K)+OP; 	// Internal OP
	     sOP[0]= ((((Value1/1024)-(SP/368))*100)*K); 	// External OP
	      sCmd[0]= 'M';
	      if(!writeport(fd, sCmd))
	         return 1;
	     usleep(500);
      ...
      float ERsum;	// Error sumatory over time
      ERsum= ERsum+(SP-PV);
      ...
      if (ustrcmp(Mode,"PI")==0)
	{
	  ...
	  if((SP-PV) =='0') 	// No Error
	   ...
	   {
	      OP= (   (((PV/368)-(SP/368))*100) + (( ((PV/368)-(SP/368)-ER2)/(Tin*M))*(ERsum)) *K);	// Internal OP
//	      OP= (   (((PV/368)-(SP/368))*100) + ((M/(2*Tin))*(2*ERsum))   *K);	// Internal OP
	      sOP[0]= ((((Value1/1024)-(SP/368))*100) + ((((PV-SP)-ER2)/(Tin*M))*(ERsum)) *K);
//	     sOP[0]= (  (((Value1/1024)-(SP/368))*100)  +  ((M/(2*Tin))*(2*ERsum))  *K); 	// External OP
	      sCmd[0]= 'M';
	      if(!writeport(fd, sCmd))
	         return 1;
	     usleep(500);
      ...
      if (ustrcmp(Mode,"PID")==0)
	{
	  ...
	  if((SP-PV) =='0') 	// No Error
	   ...
	   {
	      OP= ((((PV/368)-(SP/368))*100) + (((((PV/368)-(SP/368))-ER2)/(Tin*M))*(ERsum))+Tdn*( ((PV/368)-(SP/368))-ER3 + (3*ER1)/ER2 ) *K);
//	      OP= (   (((PV/368)-(SP/368))*100) + ((M/(2*Tin))*(2*ERsum)) + Tdn( ((PV/368)-(SP/368))-ER3 + (3*ER1)/ER2 ) *K);
	      sOP[0]=((((Value1/1024)-(SP/368))*100)+((((PV-SP)-ER2)/(Tin*M))*(ERsum))+Tdn*( ((PV/368)-(SP/368))-ER3 + (3*ER1)/ER2 ) *K);
//	     sOP[0]= (  (((Value1/1024)-(SP/368))*100)  +  ((M/(2*Tin))*(2*ERsum)) +     *K); 	// External OP
	      sCmd[0]= 'M';
	      if(!writeport(fd, sCmd))
	         return 1;
	 ...
	 ER1= ER2;
      ...
      //      if (PV>=40)
      //	{
      blit(CleanBox, screen, 0, 0, 82, 297, 894, 42); 
...
int Graphics()
{
  ...
  CleanBox = load_bitmap("images/Principal/Cleanbox.pcx", pal);
